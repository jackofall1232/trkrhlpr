...
@Composable fun IndustrialBackdrop(modifier: Modifier = Modifier, content: @Composable BoxScope.() -> Unit) {
    Box(modifier.background(MaterialTheme.colorScheme.background)) {
        Canvas(Modifier.matchParentSize().semantics { hideFromAccessibility() }) {
            drawRect(Brush.verticalGradient(listOf(Color.Transparent, WagonColors.Graphite800.copy(alpha = .28f))))
            var y = -40f
            while (y < size.height) {
                drawLine(WagonColors.MarkerAmber.copy(alpha = .1f), Offset(size.width * .86f, y),
...
@Composable fun WagonCard(
    ...
    Column((if (onClick == null) base else base.clickable(onClick = onClick)).padding(WagonSpacing.md),
        verticalArrangement = Arrangement.spacedBy(WagonSpacing.sm), content = content)
}
...
@Composable fun FeatureTile(
    ...
    WagonCard(modifier.heightIn(min = 156.dp), if (enabled) onClick else null) {
...
@Composable fun WagonProgress(value: Float, label: String, modifier: Modifier = Modifier) {
    val animated by animateFloatAsState(
        value.coerceIn(0f, 1f),
        animationSpec = if (LocalReduceMotion.current) snap() else tween(180),
        label = "progress",
    )
    Column(modifier, verticalArrangement = Arrangement.spacedBy(WagonSpacing.xs)) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
...
@Composable fun ChecklistRow(
    ...
    val border by animateColorAsState(
        if (complete) WagonColors.SignalGreen else MaterialTheme.colorScheme.outline.copy(alpha = .25f),
    ...
    Column(modifier.fillMaxWidth().clip(MaterialTheme.shapes.medium).border(1.dp, border, MaterialTheme.shapes.medium)
        ...
        Row(Modifier.fillMaxWidth().clickable(onClick = onToggleExpanded).padding(WagonSpacing.md),
            verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(WagonSpacing.sm)) {
            IconButton(onToggleComplete, Modifier.size(48.dp).semantics {
                ...
                Icon(if (complete) Icons.Rounded.CheckCircle else Icons.Rounded.RadioButtonUnchecked,
                    ...
                    tint = if (complete) WagonColors.SignalGreen else MaterialTheme.colorScheme.onSurfaceVariant)
            }
            Column(Modifier.weight(1f)) {
                Text(title, style = MaterialTheme.typography.titleMedium)
                Text(if (complete) "Complete" else "Tap to review", style = MaterialTheme.typography.labelMedium,
                    color = if (complete) WagonColors.SignalGreen else MaterialTheme.colorScheme.primary)
            }
        ...
        if (expanded) {
            ...
            Text(supporting, Modifier.padding(WagonSpacing.md), style = MaterialTheme.typography.bodyMedium,
                color = MaterialTheme.colorScheme.onSurfaceVariant)
        }
    }
}
...
@Composable fun AnswerOption(
    ...
    val color = when (correct) {
        true -> WagonColors.SignalGreen; false -> WagonColors.BrakeRed
    ...
    Row(Modifier.fillMaxWidth().heightIn(min = 56.dp).clip(MaterialTheme.shapes.small)
        ...
        .clickable(enabled = enabled, onClick = onClick).padding(WagonSpacing.md),
        verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(WagonSpacing.sm)) {
...
@Composable fun LoadingPanel(label: String = "Loading") {
    WagonCard(Modifier.fillMaxWidth().semantics { contentDescription = label }) {
